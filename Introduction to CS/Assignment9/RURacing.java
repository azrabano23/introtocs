...
public class RURacing {
    ...
    public static void racer1(char[][] map) {
        int x = 0, y = 0;
        int totalDistance = map.length * map.length;
        int[] finalPosition = moveRacer(x, y, totalDistance, map);
        System.out.printf("Racer1 is at: (%d,%d)%n", finalPosition[0], finalPosition[1]);
    }
     ...
     * @return
     */
    // Racer2: O(log n) - Double distance each step
    public static void racer2(char[][] map) {
        int x = 0, y = 0;
        int distance = 1; // Start with distance 1
        while (distance <= map.length * map.length) {
            int[] position = moveRacer(x, y, distance, map);
            System.out.printf("Racer2 is at: (%d,%d)%n", position[0], position[1]);
            x = position[0];
            y = position[1];
            distance *= 2; // Double the distance for next step
        }
    }
     ...
     * @return
     */
    // Racer3: O(n) - Move linearly across the raceway
    public static void racer3(char[][] map) {
        int x = 0, y = 0;
        int n = map.length * map.length; // Total steps to complete
        for (int i = 0; i < n; i++) {
            int[] position = moveRacer(x, y, 1, map);
            System.out.printf("Racer3 is at: (%d,%d)%n", position[0], position[1]);
            x = position[0];
            y = position[1];
        }
     ...
     * @return
     */
   // Racer4: O(n²) - Stay at each position for n steps
   public static void racer4(char[][] map) {
    int x = 0, y = 0;
    int n = map.length; // n² behavior
    for (int i = 0; i < n * n; i++) {
        for (int j = 0; j < n; j++) { // Repeat n steps per position
            int[] position = moveRacer(x, y, 1, map);
            System.out.printf("Racer4 is at: (%d,%d)%n", position[0], position[1]);
            x = position[0];
            y = position[1];
        }
    }
